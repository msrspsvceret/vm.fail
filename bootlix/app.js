class BootlixApp {
    constructor() {
        this.elixirBaseUrl = "https://elixir.bootlin.com";
 this.init();
    }
    ...
    async handleSearch(e) {
        ...
        }?version=${version}&family=C`;
        try {
            const response = await fetch(url);
            if (! response.ok) 
                throw new Error(`Elixir API returned status ${
                    response.status
                }`);
    ...
    renderResults(data) {
        const resultsDiv = document.getElementById("results");
        if (data.error) {
            resultsDiv.innerHTML = `<div class="bg-red-900 border border-red-700 rounded-lg p-4"><h3 class="text-red-400 font-semibold">Error</h3><p>${
                data.message
            }</p></div>`;
            return;
        }
        if (!data.results || data.results.length === 0) {
            resultsDiv.innerHTML = `<div class="bg-yellow-900 border border-yellow-700 rounded-lg p-4"><h3 class="text-yellow-400 font-semibold">No Results</h3><p>Sorry! Parsers couldn't find anything for <span class="font-mono">"${
                data.query
            }"</span>.</p></div>`;
            return;
        }
        ...
        let html = `<div class="bg-neutral-950 rounded-lg p-6"><h2 class="text-2xl font-bold text-blue-400 mb-4">Results for "<span class="font-mono">${
            data.query
        }</span>"</h2>`;
        if (definitions.length > 0) {
            ...
            definitions.forEach(def => {
                html += `<div class="bg-[#0f0f0f] rounded p-4 border-l-4 border-blue-400"><div class="flex justify-between items-start"><div><span class="text-blue-500 font-semibold">${
                    def.name
                }</span> <span class="text-gray-100 ml-2">(${
                    def.type
                })</span></div><div class="text-right text-sm text-gray-400"><div>${
                    def.file
                }:${
                    def.line
                }</div></div></div><button onclick="window.bootlix.showSourceCode('${
                    def.project
                }', '${
                    def.version
                }', '${
                    ...
                    def.name
                }')" class="text-blue-400 hover:text-blue-300 text-sm mt-2 font-sans">View source →</button></div>`;
            });
            html += `</div></div>`;
        }
        if (references.length > 0) {
            html += `<div><h3 class="text-lg font-semibold text-blue-200 mb-3">References (${
                references.length
            })</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono">`;
            references.slice(0, 50).forEach(ref => {
                html += `<div class="bg-[#0f0f0f] rounded p-3 border-l-4 border-blue-200"><div class="text-sm"><div class="text-gray-300">${
                    ref.file
                }:${
                    ref.line
                }</div><button onclick="window.bootlix.showSourceCode('${
                    ref.project
                }', '${
                    ref.version
                }', '${
                    ...
                    data.query
                }')" class="text-blue-400 hover:text-blue-300 text-xs mt-1 font-sans">View →</button></div></div>`;
            });
            html += `</div></div>`;
        }
        html += `</div>`;
    ...
    async showSourceCode(project, version, file, line) {
        ...
        const url = `${
            this.elixirBaseUrl
        }/${project}/${version}/source/${file}?raw=1`;
        try {
            const response = await fetch(url);
            if (! response.ok) 
                throw new Error(`Source file not found (status: ${
                    response.status
                })`);
            ...
            placeholderEl.innerHTML = `<div class="p-4 text-red-400 text-center">${
                error.message
            }</div>`;
        }
    }
