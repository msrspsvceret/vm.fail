class BootlixApp {
    ...
    async handleSearch(e) {
        ...
        if (!q) {
            resultsDiv.innerHTML =
                `<div class="text-center text-gray-400 py-8"><p>Please enter a search term.</p></div>`;
            spinner.style.display = 'none';
            return;
        }

        const url = `${this.workerBaseUrl}/search?q=${encodeURIComponent(q)}&project=${project}&version=${version}`;
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    apiData.message || `API returned status ${response.status}`);
            }
    ...
    renderResults(data) {
        ...
        if (data.error) {
            ...
            resultsDiv.innerHTML =
                `<div class="bg-red-900 border border-red-700 rounded-lg p-4"><h3 class="text-red-400 font-semibold">${data.error}</h3><p>${errorMessage}</p></div>`;
            return;
        }
        if (!data.results || data.results.length === 0) {
            resultsDiv.innerHTML =
                `<div class="bg-yellow-900 border border-yellow-700 rounded-lg p-4"><h3 class="text-yellow-400 font-semibold">No Results</h3><p>Sorry! Parsers couldn't find anything for <span class="font-mono">"${data.query}"</span>.</p></div>`;
            return;
        }
        const definitions = data.results.filter(r => r.type !== 'reference');
        const references = data.results.filter(r => r.type === 'reference');
        let html =
            `<div class="bg-neutral-950 rounded-lg p-6"><h2 class="text-2xl font-bold text-blue-400 mb-4">Results for "<span class="font-mono">${data.query}</span>"</h2>`;
        if (definitions.length > 0) {
            html +=
                `<div class="mb-6"><h3 class="text-lg font-semibold text-blue-400 mb-3">Definitions</h3><div class="space-y-2 font-mono">`;
        ...
        if (references.length > 0) {
            html += `<div><h3 class="text-lg font-semibold text-blue-200 mb-3">References (${references
                    .length})</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono">`;
    ...
    async showSourceCode(project, version, file, line) {
        ...
        document.getElementById('code-viewer-link').href =
            `https://elixir.bootlin.com/${project}/${version}/source/${file}#L${line}`;
        ...
        const url = `${this.workerBaseUrl}/api/source/${project}/${version}?path=${encodeURIComponent(file)}`;
        try {
            const response = await fetch(url);
            const data = await response.json();  // The worker always returns JSON
